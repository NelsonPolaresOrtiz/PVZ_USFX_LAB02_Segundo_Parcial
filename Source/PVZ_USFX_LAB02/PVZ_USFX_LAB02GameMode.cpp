...
#include "Nuez.h"
#include "Hongo.h"
#include "FactoryMethod_Principal.h" //Patron FactoryMethod_Principal
...
FVector APVZ_USFX_LAB02GameMode::GenerateRandomSpawnLocation()
{
	// Genera una posición aleatoria dentro de un rango específico
	FVector MinSpawnLocation = FVector(-1600.0f, 1100.0f, 200.0f);
	FVector MaxSpawnLocation = FVector(1600.0f, 2200.0f, 600.0f);

	FVector RandomSpawnLocation;
	RandomSpawnLocation.X = FMath::FRandRange(MinSpawnLocation.X, MaxSpawnLocation.X);
	RandomSpawnLocation.Y = FMath::FRandRange(MinSpawnLocation.Y, MaxSpawnLocation.Y);
	RandomSpawnLocation.Z = FMath::FRandRange(MinSpawnLocation.Z, MaxSpawnLocation.Z);
...
void APVZ_USFX_LAB02GameMode::BeginPlay()
{
	...
	//AFactoryMethod_Principal* FactoryMethod = GetWorld()->SpawnActor<AFactoryMethod_Principal>(AFactoryMethod_Principal::StaticClass(), FVector(-1600.0f, 1100.0f, 200.0f), FRotator::ZeroRotator);
	// Respawnear 5 veces el objeto AFactoryMethod_Principal
	/*for (int i = 0; i < 5; i++) {
		AFactoryMethod_Principal* FactoryMethod = GetWorld()->SpawnActor<AFactoryMethod_Principal>(AFactoryMethod_Principal::StaticClass(), FVector(-1200.0f + i * 150.0f, 100.0f, 200.0f), FRotator::ZeroRotator);
	}*/
	// Respawnear 5 veces el objeto AFactoryMethod_Principal en posiciones aleatorias
	for (int i = 0; i < 5; i++) {
		FVector RandomSpawnLocation = GenerateRandomSpawnLocation(); // Genera una posición aleatoria
		AFactoryMethod_Principal* FactoryMethod = GetWorld()->SpawnActor<AFactoryMethod_Principal>(AFactoryMethod_Principal::StaticClass(), RandomSpawnLocation, FRotator::ZeroRotator);
	}
	
		//AFactoryMethod_Principal* NewLanzaguisantes = SpawnPlantLanzaguisantes(FVector(initialPositionX + i * 150.0f, initialPositionY, 200.0f));
	//AFactoryMethod_Principal* FactoryMethod = GetWorld()->SpawnActor<AFactoryMethod_Principal>(AFactoryMethod_Principal::StaticClass(), FVector(initialPositionX + i * 150.0f, initialPositionY, 200.0f), FRotator::ZeroRotator);
	...
	for (int i = 0; i < 5; i++)
	{
		...
		//(-1500.0f+i*150.0f,100.0f,200.0f)
